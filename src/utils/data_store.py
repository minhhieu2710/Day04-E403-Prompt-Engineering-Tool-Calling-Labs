...
def _normalize(text: str) -> str:
    """Normalize text for fuzzy matching: strip accents, lowercase, collapse whitespace."""
...
class OrderDataStore:
    ...
    All methods are deterministic so the grader can reproduce exact outputs.
    """
    ...
    def build_detail_token(product_ids: list[str]) -> str:
        """Build a deterministic validation token from a sorted set of product IDs."""
    ...
    def list_products(
        ...
        *,
        ...
        """
        Search the product catalog by name, brand, category, tags, and description.
        Returns compact catalog summaries that the model can reuse in later tool calls.
        """
    ...
    def get_product_details(self, product_ids: list[str]) -> dict:
        ...
        Returns a deterministic validation token that later tools can verify.
        """
        details: list[dict] = []
    ...
    def get_discount(self, *, seed_hint: str, customer_tier: str = "standard") -> dict:
        ...
        Supported discount rates: 0.1 or 0.2.
        """
    ...
    def calculate_order_totals(self, *, items: list[OrderLineInput], detail_token: str, discount_rate: float) -> dict:
        """
        Validate product IDs, detail token, and requested quantities against stock.
        Compute subtotal, discount amount, and final total.
        Returns an error payload instead of throwing for common user mistakes.
        """
    ...
    def save_order(
        ...
        *,
        ...
        """
        Recompute totals before saving. Build a deterministic order ID.
        Persist the final JSON payload to the output directory.
        Returns both the saved order payload and the saved file path.
        """
        ...
        order_id = "ORD-" + hashlib.sha1(seed_payload.encode("utf-8")).hexdigest()[:16].upper()
